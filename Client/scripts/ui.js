// Function to remove file from fileStates
function removeFile(file) {
  let index;
  let indexOfUF;
  // If file is being uploaded from desktop, it will be File object
  // If file is coming from database it will be of type number (It's ID)
  if (file instanceof File) {
    index = fileStates.newFiles.indexOf(file)
    indexOfUF = uploadedFiles.indexOf(file)
    console.log(index, indexOfUF)
    if (index !== -1) {
      fileStates.newFiles.splice(index, 1);
      uploadedFiles.splice(indexOfUF, 1)
      console.log(fileStates)
    }
  } else {
    index = fileStates.existingFiles.indexOf(file);
    if (index !== -1) {
      fileStates.existingFiles.splice(index, 1);
      fileStates.filesMarkedForDeletion.push(file);
    }
  }
}
    ...
    pdfjsLib.getDocument(mediaSrc).promise.then(function (pdf) {
      pdf.getPage(1).then(function (page) {
        ...
        page.render(renderContext).promise.then(function () {
          ...
          if (window.location.href.includes("admin-panel")) {
